...
void GameSystem::addPlayer(int playerId, Group* groupPtr, int level)
{
    ...
    if (playersById.isInTree(PlayerById(playerId, nullptr)))
    {
...
void GameSystem::replaceGroup(int groupId, int replacementId)
{
	...
	int originalSize = replacement->getSize();
	replacement->mergeGroups(*group);
	if (originalSize == 0 && replacement->getSize() > 0)
    {
	    nonEmptyGroups.addNode(replacement);
    }
